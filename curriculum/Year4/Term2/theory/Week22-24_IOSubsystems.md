# Week 22-24: Input/Output Subsystems - Theory
...
### 4B. Matrix Keypad Interface (OPTIONAL ENHANCEMENT)

**Status**: ⚠️ **OPTIONAL** - Valuable educational progression but not required

**Matrix Keypad**:
- **4×4 keypad**: 16 keys (0-9, A-F or 0-9, +, -, *, /)
- **Matrix structure**: Rows and columns reduce wiring
- **Scanning technique**: Sequentially activate rows, read columns
- **Text/numeric input**: Can enter numbers and limited characters

**Why Optional?**:
- Good educational progression from buttons (Year 3)
- Teaches matrix scanning and debouncing
- Prepares for PS/2 keyboard (Year 6)
- **Not critical**: System works without it (buttons sufficient)

**Hardware Requirements**:
- **4×4 Matrix Keypad**: ~$3-5
- **I/O Port**: 8-bit port (4 rows + 4 columns)
- **Pull-up Resistors**: 10kΩ × 4 (for columns)
- **VIA Port**: Can use VIA Port A or B

**Matrix Scanning**:
- **Principle**: Set one row LOW, read columns
- **If column LOW**: Key at that row/column is pressed
- **Scan all rows**: Sequentially check each row
- **Debouncing**: Filter switch bounce (10-20ms delay)
...
- ✅ **Progression**: Next step after buttons, before PS/2 keyboard

**Integration**:
- Connect to VIA Port A or B
- Configure 4 bits as output (rows)
- Configure 4 bits as input (columns with pull-ups)
- Implement scanning algorithm
- Add debouncing logic

**Key Code Mapping**:
- **0-15**: Key codes for 16 keys
- **ASCII conversion**: Convert key codes to characters
- **Custom mapping**: Can map keys to specific functions

**Reference**: See `../../../resources/MATRIX_KEYPAD_INTERFACE_GUIDE.md` for comprehensive guide

**Recommendation**: **OPTIONAL** - Implement if time permits, valuable but not essential
...
- Research graphics options (Weeks 25-27)
- Research sound options (Weeks 25-27)
- Prepare for Year 5
