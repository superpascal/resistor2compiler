...
**Activities**:
1. **Prepare Etchant**:
   - Set up etching station (well-ventilated area)
   - **Select Etchant** (see etchant options below):
     - **Ferric Chloride** (Recommended - most common, cost-effective)
     - **Ammonium Persulfate** (Alternative - faster, cleaner, more expensive)
   - Prepare etchant solution:
     - **Ferric Chloride**: 30-40% solution (300-400g per liter)
       - Add ferric chloride to water slowly (exothermic reaction)
       - Warm to 40-50°C for faster etching (optional)
     - **Ammonium Persulfate**: 15-20% solution (150-200g per liter)
       - Dissolve in warm water (40-50°C)
       - Use fresh solution
   - Ensure safety equipment available (PPE, eye wash, spill containment)

2. **Etch Board**:
   - Pour etchant into **non-metallic tray** (plastic or glass)
   - Warm etchant to 40-50°C (if using warm method - speeds etching)
   - Submerge board in etchant (copper side up)
   - **Agitate continuously** (rock tray or use agitation equipment)
   ...
   - Remove board immediately when all unwanted copper is dissolved

3. **Clean Board**:
   - Rinse board thoroughly with water (stops etching)
   - Remove toner/photoresist with acetone (if toner transfer) or stripper (if presensitized)
   - Inspect traces
   - Touch up any issues (if needed)

**Safety**:
- **MANDATORY**: Gloves, safety goggles, lab coat
- **Well-ventilated area** (fumes from etchant)
- **Proper chemical handling** (no splashing, careful pouring)
- **Temperature Control**: Use water bath for warming (NOT direct heat/flame)
- **Immediate Rinse**: Rinse any skin contact immediately
- **Neutralize etchant before disposal** (DO NOT pour down drain)

**Etchant Options**:
- **Ferric Chloride (FeCl₃)**: Most common, cost-effective, reusable, stains everything
- **Ammonium Persulfate ((NH₄)₂S₂O₈)**: Faster, cleaner, more expensive, clear solution
- **See**: `../resources/PRESENSITIZED_PCB_TEACHER_GUIDE.md` for detailed etchant information and alternatives
...
- **UV Exposure Unit**: UV light source (can be DIY with UV LEDs)
  - **DIY Option**: See `../resources/UV_LED_EXPOSURE_BOX_PROJECT.md` for affordable UV LED exposure box project (~$60-80, teacher/parent-assisted, can be built by class or individual students)
  - **Reference**: Based on [affordable DIY UV LED exposure box](https://zebradryplates.com/how-i-built-myself-affordable-uv-led-exposure-box/)
...
- **Issue**: Etching too slow
  - **Solutions**: 
    - Warm etchant to 40-50°C (speeds etching significantly)
    - Increase agitation (continuous agitation required)
    - Check etchant concentration (may need to add more etchant)
    - Verify etchant is still active (ferric chloride can be reused, but loses effectiveness)
- **Issue**: Etching too fast
  - **Solutions**: 
    - Cool etchant (reduce temperature)
    - Reduce agitation
    - Monitor more closely (check every minute)
- **Issue**: Uneven etching
  - **Solutions**: 
    - Improve agitation (ensure even contact)
    - Check board is fully submerged
    - Verify etchant covers entire board
- **Issue**: Over-etching (traces damaged)
  - **Solutions**: 
    - Reduce etching time
    - Monitor progress closely
    - Remove board immediately when complete
- **Issue**: Under-etching (copper not fully removed)
  - **Solutions**: 
    - Increase etching time
    - Warm etchant (if using room temperature)
    - Increase agitation
    - Check etchant concentration

**Etchant Information**:
- **Ferric Chloride**: Most common, 30-40% solution, warm to 40-50°C for faster etching
- **Ammonium Persulfate**: Alternative, 15-20% solution, requires warm temperature (40-50°C)
